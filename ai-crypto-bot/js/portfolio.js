...
export class PortfolioManager {
  ...
  updateMarkPrices(currentPrices) {
    ...
    this.positions.forEach((pos, index) => {
      ...
      // 🛡️ Dinamik Trailing Stop & Kâr Kilitleme Motoru
      const dec = pos.entryPrice >= 1000 ? 2 : (pos.entryPrice >= 1 ? 4 : 6);
      if (pos.side === "LONG") {
        if (pos.pnlPercent >= 2.2) {
          const target = parseFloat((markPrice * 0.99).toFixed(dec));
          if (target > pos.sl) { pos.sl = target; stateChanged = true; }
        } else if (pos.pnlPercent >= 1.2) {
          const target = parseFloat((pos.entryPrice * 1.006).toFixed(dec));
          if (target > pos.sl) { pos.sl = target; stateChanged = true; }
        } else if (pos.pnlPercent >= 1.0) {
          if (pos.sl < pos.entryPrice) { pos.sl = pos.entryPrice; stateChanged = true; }
        }
      } else { // SHORT
        if (pos.pnlPercent >= 2.2) {
          const target = parseFloat((markPrice * 1.01).toFixed(dec));
          if (target < pos.sl) { pos.sl = target; stateChanged = true; }
        } else if (pos.pnlPercent >= 1.2) {
          const target = parseFloat((pos.entryPrice * 0.994).toFixed(dec));
          if (target < pos.sl) { pos.sl = target; stateChanged = true; }
        } else if (pos.pnlPercent >= 1.0) {
          if (pos.sl > pos.entryPrice) { pos.sl = pos.entryPrice; stateChanged = true; }
        }
