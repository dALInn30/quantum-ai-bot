...
class AppController {
  ...
  async syncServerState() {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data.positions)) this.portfolio.positions = data.positions;
        if (Array.isArray(data.history)) this.portfolio.history = data.history;
        if (data.balance !== undefined) this.portfolio.balance = data.balance;
        if (data.auto_pilot !== undefined) {
          this.portfolio.autoPilotEnabled = data.auto_pilot;
          const autoCheck = document.getElementById('autoPilotCheck');
          if (autoCheck) autoCheck.checked = data.auto_pilot;
        }
      }
    } catch (e) {
      // Local fallback
  ...
  async realtimePoll() {
    ...
    // 3. Fetch Latest Klines for active symbol
    await this.fetchSymbolKlines(this.activeSymbol, this.activeTimeframe);
  ...
  updatePortfolioUI() {
    ...
    if (document.getElementById('histWinRate')) {
      document.getElementById('histWinRate').textContent = `Başarı: %${this.portfolio.getWinRate()}`;
    }
    document.getElementById('posCount').textContent = this.portfolio.positions.length;
    if (document.getElementById('histCount')) {
      document.getElementById('histCount').textContent = this.portfolio.history.length;
    }

    // Refresh Signal Card Banner if currentSignal is loaded
    if (this.currentSignal) {
      const existingPos = this.portfolio.getPositionForSymbol(this.currentSignal.symbol);
      const banner = document.getElementById('positionStatusBanner');
      ...
      if (!existingPos) {
        ...
        if (btnTrade) {
          ...
          btnTrade.className = 'btn-execute';
          btnTrade.innerHTML = `
            <i class="fa-solid fa-play"></i>
            <span>Sanal İşlem Aç ($1,000 USDT)</span>
          `;
        }
      }
    }

    // 1. Render Open Positions Table
    ...
    // 2. Render Closed Trade History Table
    ...
    if (histBody) {
      if (this.portfolio.history.length === 0) {
        histBody.innerHTML = `
          <tr>
            <td colspan="6" style="text-align: center; color: var(--text-dim); padding: 16px;">
              Henüz kapanmış geçmiş işlem bulunmuyor.
            </td>
          </tr>
        `;
      } else {
        histBody.innerHTML = '';
        this.portfolio.history.forEach(hist => {
          const isWin = hist.pnl >= 0;
          const tr = document.createElement('tr');
          tr.innerHTML = `
            <td><strong>${hist.symbol}</strong></td>
            <td><span class="${hist.side === 'LONG' ? 'badge-long' : 'badge-short'}">${hist.side}</span></td>
            <td style="font-size: 11px;">${formatPrice(hist.entryPrice)} → ${formatPrice(hist.closePrice || hist.markPrice)}</td>
            <td style="color: ${isWin ? 'var(--color-bullish)' : 'var(--color-bearish)'}; font-weight: 700;">
              ${isWin ? '+' : ''}$${(hist.pnl || 0).toFixed(2)} (${isWin ? '+' : ''}${(hist.pnlPercent || 0).toFixed(2)}%)
            </td>
            <td style="font-size: 11px; max-width: 140px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${hist.closeReason || 'Kapatıldı'}">
              ${hist.closeReason || 'Kapatıldı'}
            </td>
            <td style="font-size: 10px; color: var(--text-muted);">${hist.closeTime || hist.closeTimestamp || hist.timestamp || '---'}</td>
          `;
  ...
  bindEvents() {
    ...
    if (btnTrade) {
      btnTrade.addEventListener('click', async () => {
        if (this.currentSignal) {
          const existingPos = this.portfolio.getPositionForSymbol(this.currentSignal.symbol);
          let wasReversal = false;
          if (existingPos && existingPos.side !== (this.currentSignal.side === 'BUY' ? 'LONG' : 'SHORT')) {
            wasReversal = true;
          }
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (result.success) {
                this.portfolio.openPositionFromSignal(this.currentSignal);
                this.updatePortfolioUI();
                if (wasReversal) {
                  this.logTerminalMessage(`[POZİSYON REVİZE EDİLDİ] Formasyon bozulduğu için eski pozisyon kapatıldı ve ${result.position.symbol} ${result.position.side} pozisyonu açıldı!`);
                } else {
                  this.logTerminalMessage(`[İŞLEM AÇILDI] ${result.position.symbol} için ${result.position.side} pozisyonu $${result.position.entryPrice} fiyatından açıldı ve Telegram bildirimi gönderildi!`);
                }
                return;
          ...
          if (pos) {
            this.updatePortfolioUI();
            if (wasReversal) {
              this.logTerminalMessage(`[POZİSYON REVİZE EDİLDİ] Formasyon bozulduğu için eski pozisyon kapatıldı ve ${pos.symbol} ${pos.side} pozisyonu açıldı!`);
            } else {
              this.logTerminalMessage(`[İŞLEM AÇILDI] ${pos.symbol} için ${pos.side} pozisyonu $${pos.entryPrice} fiyatından açıldı!`);
            }
          }
        }
      });
    ...
    if (saveTgBtn) {
      saveTgBtn.addEventListener('click', async () => {
        ...
        if (enabled && !token) {
          alert('Lütfen @BotFather üzerinden aldığınız Telegram Bot Token alanını doldurunuz.');
          return;
        }
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            alert('✅ Telegram Ayarları Başarıyla Kaydedildi ve Doğrulama Mesajı Gönderildi!');
            if (tgModal) tgModal.classList.remove('active');
          } else {
            alert('⚠️ Telegram Hatası:\n' + (data.message || 'Bilinmeyen hata') + '\n\n💡 ÇÖZÜM:\n1. Tokeninizin doğru olduğunu kontrol edin.\n2. Telegram uygulamanızda botunuzu açıp "Start" (Başlat) veya /start mesajı gönderin.');
          }
        } catch (e) {
    ...
    if (testTgBtn) {
      testTgBtn.addEventListener('click', async () => {
        ...
        if (!token) {
          alert('Test sinyali gönderebilmek için geçerli bir Bot Token girilmelidir.');
          return;
        }

        // Save first
        await fetch('/api/save-telegram', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ bot_token: token, chat_id: chatId, enabled: enabled })
        });
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            alert('🚀 SİNYAL BAŞARIYLA GÖNDERİLDİ!\n\nTelegram uygulamanızı kontrol ediniz. Test sinyal bildirimi cep telefonunuza ulaşmıştır.');
          } else {
            alert('⚠️ Sinyal Gönderim Hatası:\n\n' + data.message + '\n\n💡 İPUCU: Telegram botunuza telefonunuzdan tek bir kelime (örneğin /start) yazarak bota mesaj göndermelisiniz.');
          }
        } catch (e) {
          alert('Sunucu bağlantı hatası.');
        }
  ...
  updateSignalCardUI(signal) {
    ...
    // 🛡️ Mevcut Pozisyon ve Formasyon Bozulma Uyarısı UI Kontrolü
    const existingPos = this.portfolio.getPositionForSymbol(signal.symbol);
    const banner = document.getElementById('positionStatusBanner');
    const btnTrade = document.getElementById('btnExecuteTrade');

    if (existingPos) {
      const targetSide = signal.side === 'BUY' ? 'LONG' : (signal.side === 'SELL' ? 'SHORT' : 'HOLD');
      
      if (targetSide !== 'HOLD' && existingPos.side !== targetSide) {
        if (banner) {
          banner.style.display = 'block';
          banner.innerHTML = `
            <div class="pos-status-box warning">
              <i class="fa-solid fa-triangle-exclamation"></i>
              <div>
                <strong>⚠️ FORMASYON BOZULDU / TERS SİNYAL TESPİT EDİLDİ</strong>
                <br>Açık <strong>${existingPos.side}</strong> pozisyonunuz var. YZ ters yönde (<strong>${targetSide}</strong> %${signal.confidence}) sinyal algıladı. Aynı anda hem LONG hem SHORT açılamaz.
              </div>
            </div>
          `;
        }
        ...
        if (btnTrade) {
          btnTrade.disabled = false;
          btnTrade.className = 'btn-execute reverse-mode';
          btnTrade.innerHTML = `
            <i class="fa-solid fa-arrows-rotate"></i>
            <span>Mevcut ${existingPos.side} Pozisyonunu Kapat ve ${targetSide} Aç</span>
          `;
        }
      } else {
        if (banner) {
          banner.style.display = 'block';
          banner.innerHTML = `
            <div class="pos-status-box info">
              <i class="fa-solid fa-circle-check"></i>
              <div>
                <strong>✅ AKTİF POZİSYON MEVCUT</strong>
                <br>${existingPos.symbol} için $${formatPrice(existingPos.entryPrice)} fiyatından <strong>${existingPos.side}</strong> pozisyonunuz zaten açık. (Kar/Zarar: $${existingPos.pnl.toFixed(2)})
              </div>
            </div>
          `;
        }
