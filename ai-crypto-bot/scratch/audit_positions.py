...
def run_audit():
    ...
    for pos in positions:
        ...
        # Dynamic Trailing & Breakeven Stop Sync
        sl_old = pos.get("sl", entry)
        sl = sl_old
        decimals = 2 if entry >= 1000 else (4 if entry >= 1 else 6)
        sl_updated = False
        update_tag = ""

        if side == "LONG":
            if pnl_pct >= 2.2:
                target_sl = round(mark * 0.99, decimals)
                if target_sl > sl: pos["sl"] = target_sl; sl = target_sl; sl_updated = True; update_tag = "📈 İZLEYEN STOP İLE KÂR TAKİBİ"
            elif pnl_pct >= 1.2:
                target_sl = round(entry * 1.006, decimals)
                if target_sl > sl: pos["sl"] = target_sl; sl = target_sl; sl_updated = True; update_tag = "🎯 %0.6 KÂR KİLİTLENDİ"
            elif pnl_pct >= 1.0:
                if sl < entry: pos["sl"] = entry; sl = entry; sl_updated = True; update_tag = "🛡️ BAŞABAŞ STOP (RİSK SIFIRLANDI)"
        else:
            if pnl_pct >= 2.2:
                target_sl = round(mark * 1.01, decimals)
                if target_sl < sl: pos["sl"] = target_sl; sl = target_sl; sl_updated = True; update_tag = "📈 İZLEYEN STOP İLE KÂR TAKİBİ"
            elif pnl_pct >= 1.2:
                target_sl = round(entry * 0.994, decimals)
                if target_sl < sl: pos["sl"] = target_sl; sl = target_sl; sl_updated = True; update_tag = "🎯 %0.6 KÂR KİLİTLENDİ"
            elif pnl_pct >= 1.0:
                if sl > entry: pos["sl"] = entry; sl = entry; sl_updated = True; update_tag = "🛡️ BAŞABAŞ STOP (RİSK SIFIRLANDI)"

        if sl_updated and sl != sl_old:
            sl_msg = (
                f"🚨 *STOP LOSS LEVEL REVISED / GÜNCELLENDİ*\n"
                f"---------------------------------\n"
                f"🎯 *Varlık:* *{sym}* ({side})\n"
                f"📍 *Giriş Fiyatı:* `{format_price(entry)}` | Anlık Fiyat: `{format_price(mark)}`\n"
                f"🛑 *Eski Stop (SL):* `{format_price(sl_old)}` (Revize Edildi)\n"
                f"⚡ *YENİ STOP LOSS (SL):* `{format_price(sl)}` 👈\n"
                f"📊 *Güncelleme Sebebi:* *{update_tag}*\n"
                f"📈 *Güncel Kâr/Zarar:* *${pnl:+.2f} ({pnl_pct:+.2f}%)*\n"
                f"---------------------------------\n"
                f"⚠️ *Lütfen harici borsadaki Stop Loss seviyenizi `{format_price(sl)}` olarak revize ediniz.*"
            )
            send_telegram(sl_msg)

        sl_tag = f"🛡️ Başabaş/İzleyen Stop ({format_price(sl)})" if (sl > entry if side == "LONG" else sl < entry) else f"`{format_price(sl)}`"

        # Check Automated SL / Invalidation Trigger
        is_closed = False
        close_reason = ""
        ...
        if side == "LONG":
            if mark <= sl:
                is_closed = True
                close_reason = "🎯 İZLEYEN STOP İLE KÂR KİLİTLENDİ!" if sl > entry else "🛑 STOP LOSS (SL) TETİKLENDİ!"
            elif info and info["short_score"] >= 90:
                is_closed = True
                close_reason = f"🛑 FORMASYON BOZULDU (Ayı skoru: %{info['short_score']})"
        else: # SHORT
            if mark >= sl:
                is_closed = True
                close_reason = "🎯 İZLEYEN STOP İLE KÂR KİLİTLENDİ!" if sl < entry else "🛑 STOP LOSS (SL) TETİKLENDİ!"
            elif info and info["long_score"] >= 90:
                is_closed = True
                close_reason = f"🛑 FORMASYON BOZULDU (Boğa skoru: %{info['long_score']})"

        if is_closed:
            return_amount = (entry * pos["size"]) + pnl
            balance += return_amount
            hist_entry = dict(pos)
            hist_entry["closeReason"] = close_reason
            hist_entry["closePrice"] = mark
            hist_entry["closeTime"] = time.strftime("%H:%M:%S")
            history.insert(0, hist_entry)
            closed_items.append(f"• *{sym}* ({side}): {close_reason} -> Pozisyon KAPATILDI (PnL: `${pnl:+.2f}`).")
        else:
